...
public class PhoneNumberSort {
    ...
    private static void mergeFiles() {
        ...
        try {
            ...
            List<String> phoneNums = new LinkedList<String>(readerMap.keySet());
            while (readerMap.size() > 0) {
                // 排序，将最大的手机号从phoneNums集合中移除，并写入到输出文件中
                Collections.sort(phoneNums, StringComparator.getInstance());
                String phoneNum = phoneNums.remove(0);
                writer.append(phoneNum).append('\n');

                // 从readerMap中移除刚才得出的最大手机号,并从该reader中读取下一个手机号存到List和Map中，继续循环写入
                BufferedReader reader = readerMap.remove(phoneNum);
                String nextPhoneNum = reader.readLine();
                if (nextPhoneNum != null) {
     ...
     */
    private static void sortAndOutputFile(List<String> phoneNums) {
        long startTime = System.currentTimeMillis();
        Collections.sort(phoneNums, StringComparator.getInstance());
