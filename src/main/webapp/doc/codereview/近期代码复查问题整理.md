...
从上面的重构示例来看，消除重复代码会让你的代码结构发生本质的变化。发现和解决重复并不困难，通过提取抽象、提取方法等措施就能消除重复，但困难的是立即行动去解决重复，从而不断的磨砺和提升自己的编程技艺，不断将私人代码变成公共代码，这才是自我提升的过程。解决了重复，经过一段时间，你就发现对整个系统的理解程度，在不知不觉中提高了不少。