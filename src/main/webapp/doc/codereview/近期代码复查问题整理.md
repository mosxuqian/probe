...
lshi.setCBh(ArteryUtil.getUuid());
...
//        String clfxajlbUrl = "http://172.16.193.190:8089/clfx";
    ...
    String cBhAj = ArteryParamUtil.getString("CBhAj");
    String cBhXg = ArteryParamUtil.getString("CBhXg");
    Integer stlb = ArteryParamUtil.getInteger("stlb");
    ...
    try {
        ...
        stwj.setCBz(ArteryParamUtil.getString("StringCBz"));
        //是否上传建议书_材料分析_信息回填业务
        Boolean isClfxXxht = ArteryParamUtil.getBoolean("isClfxXxht");
...
//            paramMap.put("NStlb", ArteryParamUtil.getString("stlb"));
...
以上的代码也许就是大家经常写的代码，很真实，方法快接近一百行了，很多项目中几百多行的代码也有很多。甚至很多人都把责任推给了`Artery`，说是由于`Artery`引起的Logic类、方法代码量过多。实在是替`Artery`喊冤啊。咱们的代码结构普遍采用了三层结构的划分方式，即`Logic`、`service`和`dao`层，每一层的职责也是不同的，对应`Logic`层所需要做的应该是输入输出和简单必要的数据校验。根据实际情况，数据校验工作甚至都可以提取到`service`层中，通过再拆分类或者方法的方式来完成业务处理操作，从而保持接口、类、方法的职责尽可能的单一。使得让改动一处的地方对其他地方的影响尽可能的小，而且代码能被最大程度的复用。因此这里面就牵涉出了单一职责原则的设计原则。
...
public static List<File> getUploadFile(String raw) {
    ...
    if (raw != null) {
        String folder = ArteryWebUtil.getWebPath() + "/artery/downloads/"
                + raw;
...
以上的代码是用来获取Artery上传下载目录中的文件，开发自己实现，且多处重复copy使用，这个实属缺乏套路，使用一句话调用即可：
...
```java
// com.thunisoft.gjyjda.artery.sykz.Zsxq.java
...
public List<Map<String,Object>> queryListByDate(String userId, Date start, Date end) {
    ...
    return arteryDao.jdbcTemplate.queryForList(sql, new Object[]{userId, start, end});
}
```
