...
![死锁示意图](http://static.blinkfox.com/javahxjs_18_01.png)
...
![打印结果](http://static.blinkfox.com/javahxjs_18_02.png)
...
![死锁代码片段](http://static.blinkfox.com/javahxjs_18_03.png)
...
![closeLock示意图](http://static.blinkfox.com/javahxjs_18_04.png)
...
![对象（方法）和锁关系图](http://static.blinkfox.com/javahxjs_18_05.png)

- 然后根据对象之间组合、调用的关系对比和组合，考虑可能调用时序。

![对象之间组合、调用的关系对比和组合图](http://static.blinkfox.com/javahxjs_18_06.png)

- 按照可能时序合并，发现可能死锁的场景。

![时序合并、死锁的场景](http://static.blinkfox.com/javahxjs_18_07.png)
...
2. 课后题提出的死循环死锁可以认为是自旋锁死锁的一种，其他线程因为等待不到具体的信号提示。导致线程一直饥饿。这种情况下可以查看线程 cpu 使用情况，排查出使用 cpu 时间片最高的线程，再打出该线程的堆栈信息，排查代码。
