...
大致按照模式的应用目标分类，设计模式可以分为**创建型模式**、**结构型模式**和**行为型模式**。
...
这个问题主要是考察你对设计模式的了解和掌握程度，更多相关内容你可以参考[这里](https://en.wikipedia.org/wiki/Design_Patterns)。
...
![InputStream 的装饰模式](http://static.blinkfox.com/javahxjs_14_01.png)
...
举例:`JUnitCore`是JUnit类的`Facade`模式的实现类，外部使用该代理对象与JUnit进行统一交互，驱动执行测试代码。
