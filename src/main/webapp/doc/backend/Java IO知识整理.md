...
- 单一职责原则：每一个IO类的都具有单独的功能和职责。
- 里氏替换原则：子类完全实现了父类的方法，且有自己的个性。
- 接口隔离原则：输入输出流所实现的接口有所不同，而且接口更加细化，接口中的方法少。
- 依赖倒置原则：Java IO中的各个具体的功能类，都继承或聚合了`InputStream/OutputStream`的抽象类。
- 开闭原则：
